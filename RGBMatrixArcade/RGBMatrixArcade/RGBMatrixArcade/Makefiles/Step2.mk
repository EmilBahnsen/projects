...
else ifeq ($(BUILD_CORE),samd)
		$(call SHOW,"7.20-LINK",$@,.)
		$(call TRACE,"7-LINK",$@,.)
		$(CXX) $(LDFLAGS) $(OUT_PREPOSITION)$@ -L$(OBJDIR) -Wl,--start-group  $(LOCAL_OBJS) $(TARGET_A) -Wl,--end-group -lm -lgcc
...
$(OBJDIR)/%.bin2: $(OBJDIR)/%.elf
	$(call SHOW,"8.4-COPY",$@,$<)
	$(call TRACE,"8-COPY",$@,$<)
	$(UPLOADER_EXEC) -eo $< -bo Builds/$(TARGET)_00000.bin -bs .text -bs .data -bs .rodata -bc -ec -eo $< -es .irom0.text Builds/$(TARGET)_40000.bin -ec
...
ifeq ($(AVRDUDE_NO_SERIAL_PORT),1)
		...
		osascript -e 'tell application "Terminal" to do script "$(SERIAL_COMMAND) $$(ls $(BOARD_PORT)) $(SERIAL_BAUDRATE) && exit"'

else ifeq ($(UPLOADER),lightblue_loader)
		osascript -e 'tell application "Terminal" to do script "$(SERIAL_COMMAND) $$(ls $(BOARD_PORT)) $(SERIAL_BAUDRATE) && exit"'

else
		osascript -e 'tell application "Terminal" to do script "$(SERIAL_COMMAND) $(USED_SERIAL_PORT) $(SERIAL_BAUDRATE) && exit"'
endif
