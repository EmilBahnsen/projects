...
// Lavet af Martin :D
void Adafruit_PCD8544::WriteString(uint16_t x, uint16_t y, String message, bool inverse)
{
  if (inverse) // White background 
  {
    for (int i = 0; i < message.length(); i++) 
    {
      drawChar(x + (6 * i), y, message[i], WHITE, BLACK, 1);
    }
  }
  else // Black background
  {
    for (int i = 0; i < message.length(); i++) 
    {
      drawChar(x + (6 * i), y, message[i], BLACK, WHITE, 1);
    }
  }
}